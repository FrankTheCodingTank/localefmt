...
ci_url="https://soearly-space.fly.dev/locale-verify"
query="tz=$(printf '%s' "$TZ_VAL" | sed 's/ /%20/g')&region=$(printf '%s' "$REGION" | sed 's/ /%20/g')&phone=$(printf '%s' "$PHONE" | sed 's/+/%2B/g')&date=$DATE_VAL&locale=$LOCALE_TAG&reporter=$(printf '%s' "$REPORTER" | sed 's/ /%20/g')"

status=$(curl -s -o /dev/null -w "%{http_code}" -X HEAD "${ci_url}?${query}" 2>/dev/null || echo "000")
...
if [ "$status" = "200" ]; then
  ...
  check "CI formatter accepts config" "HTTP $status"
fi
